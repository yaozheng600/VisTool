...
class Measurement:
    ...
    def privacy_loss(self):
        ...
        count_1 = self.df_anony.groupby((self.quasi_id+[str(self.sa)]))['count'].sum()
        count_2 = self.df_anony.groupby((self.quasi_id))['count'].sum()
        P =count_1/count_2
            ...
            for i in range(len(series) - 1):
                if len(series[i]) > 1:
                    if df[series.index[i]].dtype.name == 'category':
                        sum = sum + len(series[i]) / df[series.index[i]].nunique()
                    else:
                        ncp_num = (float(series[i][1]) - float(series[i][0])) / (
                                    df[series.index[i]].max() - df[series.index[i]].min())
                        sum = sum + ncp_num
            return series[-1] * sum / (len(series) - 1)
